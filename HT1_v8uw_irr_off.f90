...
! real(kind=8) :: w = 3d-1/yr2sec ! m/s spreading rate
real(kind=8) :: presw_0 = 25d6 !  Pa assuming 2.5 km depth 
! real(kind=8) :: presw_0 = 50d6 !  Pa assuming 5.0 km depth 
! real(kind=8) :: presw_0 = 10d6 !  Pa assuming 1.0 km depth 
real(kind=8) :: permb = 10d0**(-16.8d0)  ! base permeability [m2] 
real(kind=8) :: permt = 10d0**(-11.3d0)  ! top permeability [m2] 
! real(kind=8) :: zhalf = 300d0 ! scale depth for half decrease of permeability (see Fisher 1998) ; default
real(kind=8) :: zhalf = 500d0 ! case 2 and 3
...
! real(kind=8),parameter :: temp_crit = 800d0
...
real(kind=8)::max_pres_dif=10d6  ! Pa (from 103 to 105 within 10 Ma from Fisher and Becker 2000)
...
real(kind=8) obshflx
logical :: offexp = .false.  ! switch to impose additional sediment pressure
! logical :: offexp = .true.
! 
...
workdir = trim(adjustl(workdir))//'perm_expexp_-16_8-11_3_zh500_spx1_200x320_irr'
