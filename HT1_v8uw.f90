...
real(kind=8) dx, dy, dt, time  ! m,m, sec, sec
...
real(kind=8) :: zscale = 100d0 ! scale depth for half decrease of permeability (see Fisher 1998) 
integer(kind = 4) ix,iy
real(kind=8) flxt, flxadv, flxcnd, flxres, flxb
! matrix solver
integer(kind = 4) nmx 
        ...
        if (iy == 1) then 
            if (ix == 1) then 
                bx(row) = & 
                    ...
                    - presw/dy)/dy
                tmprl(1) = &
                    + perm(ix,iy)/((visc(ix,iy)+viscf)*0.5d0)*(1d0/dy)/dy
                
                if (cd2(ix+1,iy) == 1) then 
                    col = cd(ix+1,iy) 
                    tmprl(1) = tmprl(1) &
                        -(perm(ix,iy)+perm(ix+1,iy))/(visc(ix,iy)+visc(ix+1,iy))*(-1d0)/dx/dx
                    
                    cnt2 = cnt2+1
                    tmprl(cnt2) = (perm(ix,iy)+perm(ix+1,iy))/(visc(ix,iy)+visc(ix+1,iy))*(-1d0)/dx/dx
                    tmpint(cnt2) = col
                ...
                if (cd2(ix,iy+1) == 1) then 
                    col = cd(ix,iy+1) 
                    tmprl(1) = tmprl(1)  &
                        -(perm(ix,iy)+perm(ix,iy+1))/(visc(ix,iy)+visc(ix,iy+1))*(-1d0)/dy/dy
                    bx(row) = bx(row) &
                        -(perm(ix,iy)+perm(ix,iy+1))/(visc(ix,iy)+visc(ix,iy+1))*grav*(rho(ix,iy+1)+rho(ix,iy))*0.5d0/dy
                        
                    cnt2 = cnt2 + 1
                    tmprl(cnt2)  = (perm(ix,iy)+perm(ix,iy+1))/(visc(ix,iy)+visc(ix,iy+1))*(-1d0)/dy/dy
                    tmpint(cnt2) = col
                ...
                if (cd2(ix,iy+1) == 1) then 
                    ...
                    tmprl(1) = tmprl(1)  &
                        -(perm(ix,iy)+perm(ix,iy+1))/(visc(ix,iy)+visc(ix,iy+1))*(-1d0)/dy/dy
                    bx(row) = bx(row)  &
                        -(perm(ix,iy)+perm(ix,iy+1))/(visc(ix,iy)+visc(ix,iy+1))*grav*(rho(ix,iy)+rho(ix,iy+1))*0.5d0/dy
            ...
            if (ix == 1) then 
                ...
                if (cd2(ix-1,iy) == 1) then 
                    col = cd(ix-1,iy) 
                    tmprl(1) = (perm(ix,iy)+perm(ix-1,iy))/(visc(ix,iy)+visc(ix-1,iy))*(1d0)/dx/dx
    ...
    do ix = 1, nx
        if (iy/=1) then 
            if (cd2(ix,iy)==0 .and.cd2(ix,iy-1)==1) then 
                pres(ix,iy) = pres(ix,iy-1)-dy*grav*(rho(ix,iy)+rho(ix,iy-1))*0.5d0
            endif
        endif 
        if (iy/=ny) then 
            if (cd2(ix,iy)==0 .and.cd2(ix,iy+1)==1) then 
...
do ix = 2, nx
    vx(ix,:) = -(perm(ix,:)+perm(ix-1,:))*0.5d0/((visc(ix,:)+visc(ix-1,:))*0.5d0)*(pres(ix,:)-pres(ix-1,:))/dx
enddo

vy(:,1) = -(perm(:,1)+perm(:,1))*0.5d0/((visc(:,1)+viscf)*0.5d0)*((pres(:,1)-presw)/dy + grav*(rho(:,1)+rhof)*0.5d0)
do iy = 2, ny
    vy(:,iy) = -(perm(:,iy)+perm(:,iy-1))*0.5d0/((visc(:,iy)+visc(:,iy-1))*0.5d0)*((pres(:,iy)-pres(:,iy-1))/dy  &
...
do iy = 1, ny
	if (iy == 1) cycle
	qvx(1,iy) = qvx(1,iy-1) + dy*(vx(1,iy-1)+vx(2,iy)+vx(1,iy)+vx(2,iy-1))/4.0d0
	do ix = 1, nx
		if (ix == 1) cycle
		qvx(ix,iy) = qvx(ix-1,iy) - dx*(vy(ix-1,iy) + vy(ix-1, iy+1) &
...
do ix = 1, nx
	if (ix == 1) cycle
	qvy(ix,1) = qvy(ix-1,1) - dx*(vy(ix-1,1) + vy(ix, 2)+vy(ix,1) + vy(ix-1, 2))/4.0d0
	do iy = 1, ny
		if (iy == 1) cycle
		qvy(ix,iy) = qvy(ix,iy-1) + dy*(vx(ix,iy-1) + vx(ix+1,iy-1)  &
        ...
        if (iy == 1) then 
            if (ix==1) then 
                bx(row) = (poro(ix,iy)*rho(ix,iy)*cp(ix,iy)+(1d0-poro(ix,iy))*rhom*cpm)*(-tempk(ix,iy))/dt &
                    ...
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)*(-(temps + 273.15d0))/dy &
                    - kappa*(temps + 273.15d0)/dy/dy  &
                    - rhom*w*(tempm+273.15d0)*cpm/dx 
                tmprl(1) = (poro(ix,iy)*rho(ix,iy)*cp(ix,iy)+(1d0-poro(ix,iy))*rhom*cpm)*(1d0)/dt &
                    ! + (vmx(ix,iy)+abs(vmx(ix,iy)))*0.5d0*(cp(ix,iy)-cpm)/dx*(1d0)  &
                    + (vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*(cp(ix+1,iy)-cp(ix,iy))/dx*(1d0)  &
                    ! + (vmx(ix,iy) - 0d0)*cp(ix,iy)/dx*(1d0)  &
                    ! + (vmx(ix,iy)+abs(vmx(ix,iy)))*0.5d0*cp(ix,iy)/dx*(1d0)  &
                    + (vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*cp(ix,iy)/dx*(-1d0)  &
                    ! - kappa*(-2d0)/dx/dx   &
                    - kappa*(-1d0)/dx/dx   &
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(cp(ix,iy)-cpf)/dy*(1d0) & 
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(cp(ix,iy+1)-cp(ix,iy))/dy*(1d0) & 
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)/dy*(1d0)  &
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)/dy*(-1d0)  &
                    - kappa*(-2d0)/dy/dy     &
                    - rhom*w*(-1d0)*cpm/dx 
                tmprl(2) =  (vmx(ix+1,iy)+abs(vmx(ix+1,iy)))*0.50d0*cp(ix+1,iy)*(-1d0)/dx   &
                    - kappa*(1d0)/dx/dx  
                tmprl(3) = (vmy(ix,iy+1)+abs(vmy(ix,iy+1)))*0.5d0*cp(ix,iy+1)/dy*(-1d0)  &
                    - kappa*(1d0)/dy/dy 
                ...
                bx(row) = (poro(ix,iy)*rho(ix,iy)*cp(ix,iy)+(1d0-poro(ix,iy))*rhom*cpm)*(-tempk(ix,iy))/dt &
                    ...
                    - kappa*(temps + 273.15d0)/dy/dy  
                tmprl(1) = (poro(ix,iy)*rho(ix,iy)*cp(ix,iy)+(1d0-poro(ix,iy))*rhom*cpm)*(1d0)/dt &
                    + (vmx(ix,iy)+abs(vmx(ix,iy)))*0.5d0*(cp(ix,iy)-cp(ix-1,iy))/dx*(1d0)  &
                    ...
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)/dy*(-1d0) & 
                    - kappa*(-2d0)/dy/dy   
                tmprl(2) =  (vmx(ix-1,iy)-abs(vmx(ix-1,iy)))*0.5d0*cp(ix-1,iy)/dx*(1d0) &
                    - kappa*(1d0)/dx/dx  
                tmprl(3) = (vmy(ix,iy+1)+abs(vmy(ix,iy+1)))*0.5d0*cp(ix,iy+1)/dy*(-1d0)  &
                    - kappa*(1d0)/dy/dy 
                ...
                tmprl(2) = (vmx(ix-1,iy)-abs(vmx(ix-1,iy)))*0.5d0*cp(ix-1,iy)/dx*(1d0)   &
                    - kappa*(1d0)/dx/dx  
                tmprl(3) = (vmx(ix+1,iy)+abs(vmx(ix+1,iy)))*0.5d0*cp(ix+1,iy)/dx*(-1d0)   &
                    - kappa*(1d0)/dx/dx  
                tmprl(4) = (vmy(ix,iy+1)+abs(vmy(ix,iy+1)))*0.5d0*cp(ix,iy+1)/dy*(-1d0)  &
                    - kappa*(1d0)/dy/dy 
            ...
            if (ix==1) then 
                ...
                tmprl(1) = (poro(ix,iy)*rho(ix,iy)*cp(ix,iy)+(1d0-poro(ix,iy))*rhom*cpm)*(1d0)/dt &
                    ...
                    ! - kappa*(-2d0)/dx/dx   &
                    - kappa*(-1d0)/dx/dx   &
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(cp(ix,iy)-cp(ix,iy-1))/dy*(1d0) &
            ...
            if (ix==1) then 
                ...
                tmprl(4) = (vmy(ix,iy-1)-abs(vmy(ix,iy-1)))*0.5d0*cp(ix,iy-1)/dy*(1d0) &
                    - kappa*(1d0)/dy/dy 
                tmprl(5) = (vmy(ix,iy+1)+abs(vmy(ix,iy+1)))*0.5d0*cp(ix,iy+1)/dy*(-1d0) &
                    - kappa*(1d0)/dy/dy 
        ...
        if (iy == 1) then 
            if (ix==1) then 
                ...
                flxadv = flxadv & 
                    ...
                    + (vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*(cp(ix+1,iy)-cp(ix,iy))*tempk(ix,iy)/dx  &
                    ! + (vmx(ix,iy) - 0d0)*cp(ix,iy)*tempk(ix,iy)/dx  &
                    + (vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*cp(ix,iy)*(tempk(ix+1,iy)-tempk(ix,iy))/dx  &
                    ! 
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(cp(ix,iy)-cpf)*tempk(ix,iy)/dy & 
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(cp(ix,iy+1)-cp(ix,iy))*tempk(ix,iy)/dy & 
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)*(tempk(ix,iy)-(temps+273.15d0))/dy  &
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)*(tempk(ix,iy+1)-tempk(ix,iy))/dy  
                    
                flxcnd = flxcnd & 
                    - kappa*(tempk(ix+1,iy)-tempk(ix,iy))/dx/dx  &
                    - kappa*(tempk(ix,iy+1)+(temps+273.15d0)-2d0*tempk(ix,iy))/dy/dy
                
                flxb = flxb &
                    - rhom*w*((tempm+273.15d0)-tempk(ix,iy))*cpm/dx 
                ...
                flxadv = flxadv & 
                    + (vmx(ix,iy)+abs(vmx(ix,iy)))*0.5d0*(cp(ix,iy)-cp(ix-1,iy))*tempk(ix,iy)/dx  &
                    ! + (vmx(ix,iy) - vmx(ix-1,iy))*cp(ix,iy)*tempk(ix,iy)/dx  &
                    + (vmx(ix,iy)+abs(vmx(ix,iy)))*0.5d0*cp(ix,iy)*(tempk(ix,iy)-tempk(ix-1,iy))/dx  &
                    !
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(cp(ix,iy)-cpf)*tempk(ix,iy)/dy & 
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(cp(ix,iy+1)-cp(ix,iy))*tempk(ix,iy)/dy & 
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)*(tempk(ix,iy)-(temps+273.15d0))/dy  &
                ...
                flxcnd = flxcnd & 
                    - kappa*(tempk(ix-1,iy)+tempk(ix+1,iy)-2d0*tempk(ix,iy))/dx/dx  &
                    - kappa*(tempk(ix,iy+1)+(temps+273.15d0)-2d0*tempk(ix,iy))/dy/dy
            ...
            if (ix==1) then 
                ...
                flxadv = flxadv & 
                    ...
                    + (vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*cp(ix,iy)*(tempk(ix+1,iy)-tempk(ix,iy))/dx  &
                    !
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(cp(ix,iy)-cp(ix,iy-1))*tempk(ix,iy)/dy & 
                    ! + (vmy(ix,iy)-vmy(ix,iy-1))*cp(ix,iy)*tempk(ix,iy)/dy  &
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)*(tempk(ix,iy)-tempk(ix,iy-1))/dy  
                    
                flxcnd = flxcnd & 
                    - kappa*(tempk(ix+1,iy)-1d0*tempk(ix,iy))/dx/dx  &
                    - kappa*(tempk(ix,iy-1)-1d0*tempk(ix,iy))/dy/dy
            ...
            if (ix==1) then 
                ...
                flxcnd = flxcnd & 
                    - kappa*(tempk(ix+1,iy)-1d0*tempk(ix,iy))/dx/dx  &
                    - kappa*(tempk(ix,iy+1)+tempk(ix,iy-1)-2d0*tempk(ix,iy))/dy/dy
...
print'(5E9.2)',flxt, flxadv, flxcnd, flxb, flxres

if (flgesc) then 
    if (abs(flxt/flxb) < 1d-5) then 
        open(unit=100,file=trim(adjustl(workdir))//'log.txt',action='write',status='unknown')
        write(100,*)"!!! Enough iteration? Tentative end !!!",it,flxt, flxadv, flxcnd, flxb
        close(100)
        exit 
    endif
endif
