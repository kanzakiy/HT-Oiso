...
real(kind=8) dx, dy, dt, time  ! m,m, yr, yr
...
real(kind=8) rflxadv, rflxrxn, rflxt, pflxadv, pflxrxn, pflxdif, pflxt 
real(kind=8) rflxrxnpls, rflxrxnmns
! matrix solver
integer(kind = 4) nmx 
...
workdir = 'C:/Users/YK/Desktop/HT_res/perm_expexp_-16_8-11_8_zh300_spx1_wd5km_200x320-20181217/'
...
do iy=1,ny
    do ix = 1,nx
        ...
        if (ix==1) then 
            ...
            bx(row)= &
                ! + (1d0-poro(ix,iy))*rhom*ms*(frx(ix,iy)-fr(ix,iy))/dt &
                + (1d0-poro(ix,iy))*rhom*ms*w*(frx(ix,iy)-frx(ix-1,iy))/dx   &
                + rhob(ix,iy)*ms*mw*kex(ix,iy)*rxn(ix,iy) 
            tmprl(1) = &
                ! + (1d0-poro(ix,iy))*rhom*ms*(1d0)/dt &
                + (1d0-poro(ix,iy))*rhom*ms*w*(1d0)/dx   &
        ...
        if (iy == 1) then 
            if (ix==1) then 
                bx(row)= &
                    ...
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*(fpx(ix+1,iy)-fpx(ix,iy))/dx  &
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(fpx(ix,iy)-fsw)/dy  & 
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(fpx(ix,iy+1)-fpx(ix,iy))/dy  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix+1,iy)-1d0*fpx(ix,iy))/dx/dx  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix,iy+1)+fsw-2d0*fpx(ix,iy))/dy/dy  & 
                    - rhob(ix,iy)*ms*mw*kex(ix,iy)*rxn(ix,iy) 
                tmprl(1) = &
                    ! + poro(ix,iy)*rho(ix,iy)*mw*(1d0)/dt   &
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmx(ix,iy)+abs(vmx(ix,iy)))*0.5d0*(1d0)/dx  & 
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*(-1d0)/dx  &
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(1d0)/dy  & 
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(-1d0)/dy  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(-1d0)/dx/dx  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(-2d0)/dy/dy  & 
                    - rhob(ix,iy)*ms*mw*kex(ix,iy)*drxn_dfp(ix,iy) 
                tmprl(2) =  & 
                    + rhob(ix,iy)*ms*mw*kex(ix,iy)*drxn_dfp(ix,iy)
                tmprl(3) =    &
                    + poro(ix+1,iy)*rho(ix+1,iy)*mw*(vmx(ix+1,iy)+abs(vmx(ix+1,iy)))*0.5d0*(-1d0)/dx  & 
                    - poro(ix+1,iy)*rho(ix+1,iy)*mw*dif(ix+1,iy)*(1d0)/dx/dx  &
                    - mw*(poro(ix+1,iy)*rho(ix+1,iy)*dif(ix+1,iy)-poro(ix,iy)*rho(ix,iy)*dif(ix,iy))*(-1d0)/dx/dx  
                tmprl(4) = &
                    + poro(ix,iy+1)*rho(ix,iy+1)*mw*(vmy(ix,iy+1)+abs(vmy(ix,iy+1)))*0.5d0*(-1d0)/dy  &
                    - poro(ix,iy+1)*rho(ix,iy+1)*mw*dif(ix,iy+1)*(1d0)/dy/dy  &
                    - mw*(poro(ix,iy+1)*rho(ix,iy+1)*dif(ix,iy+1)-poro(ix,iy)*rho(ix,iy)*dif(ix,iy))*(-1d0)/dy/dy 
                ...
                bx(row)= &
                    ! + poro(ix,iy)*rho(ix,iy)*mw*(fpx(ix,iy)-fp(ix,iy))/dt   &
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmx(ix,iy)+abs(vmx(ix,iy)))*0.5d0*(fpx(ix,iy)-fpx(ix-1,iy))/dx  & 
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(fpx(ix,iy)-fsw)/dy  & 
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(fpx(ix,iy+1)-fpx(ix,iy))/dy  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix-1,iy)-1d0*fpx(ix,iy))/dx/dx  & 
                    - mw*(poro(ix,iy)*rho(ix,iy)*dif(ix,iy)-poro(ix-1,iy)*rho(ix-1,iy)*dif(ix-1,iy)) &
                        *(fpx(ix,iy)-fpx(ix-1,iy))/dx/dx  &
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix,iy+1)+fsw-2d0*fpx(ix,iy))/dy/dy  &  
                    - rhob(ix,iy)*ms*mw*kex(ix,iy)*rxn(ix,iy) 
                tmprl(1) = &
                    ! + poro(ix,iy)*rho(ix,iy)*mw*(1d0)/dt   &
                    ...
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(1d0)/dy  & 
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(-1d0)/dy  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*(-1d0)/dx/dx  & 
                    - mw*(poro(ix,iy)*rho(ix,iy)*dif(ix,iy)-poro(ix-1,iy)*rho(ix-1,iy)*dif(ix-1,iy))*(1d0)/dx/dx  &
                    - poro(ix,iy)*rho(ix,iy)*mw*(-2d0)/dy/dy  & 
                    - rhob(ix,iy)*ms*mw*kex(ix,iy)*drxn_dfp(ix,iy) 
                tmprl(2) =  & 
                    + rhob(ix,iy)*ms*mw*kex(ix,iy)*drxn_dfp(ix,iy)
                tmprl(3) =    &
                ...
                bx(row)= &
                    ...
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(fpx(ix,iy)-fsw)/dy  & 
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(fpx(ix,iy+1)-fpx(ix,iy))/dy  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix-1,iy)+fpx(ix+1,iy)-2d0*fpx(ix,iy))/dx/dx  & 
                    - mw*(poro(ix,iy)*rho(ix,iy)*dif(ix,iy)-poro(ix-1,iy)*rho(ix-1,iy)*dif(ix-1,iy))  &
                        *(fpx(ix,iy)-fpx(ix-1,iy))/dx/dx  &
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix,iy+1)+fsw-2d0*fpx(ix,iy))/dy/dy  & 
                    - rhob(ix,iy)*ms*mw*kex(ix,iy)*rxn(ix,iy) 
                tmprl(1) = &
                    ...
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(1d0)/dy  & 
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(-1d0)/dy  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(-2d0)/dx/dx  & 
                    - mw*(poro(ix,iy)*rho(ix,iy)*dif(ix,iy)-poro(ix-1,iy)*rho(ix-1,iy)*dif(ix-1,iy))*(1d0)/dx/dx  &
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(-2d0)/dy/dy  & 
                    - rhob(ix,iy)*ms*mw*kex(ix,iy)*drxn_dfp(ix,iy) 
                tmprl(2) =  & 
            ...
            if (ix==1) then 
                bx(row)= &
                    ! + poro(ix,iy)*rho(ix,iy)*mw*(fpx(ix,iy)-fp(ix,iy))/dt   &
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*(fpx(ix+1,iy)-fpx(ix,iy))/dx  &
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(fpx(ix,iy)-fpx(ix,iy-1))/dy  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix+1,iy)-1d0*fpx(ix,iy))/dx/dx  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix,iy-1)-1d0*fpx(ix,iy))/dy/dy  & 
                    - mw*(poro(ix,iy)*rho(ix,iy)*dif(ix,iy)-poro(ix,iy-1)*rho(ix,iy-1)*dif(ix,iy-1))  &
                        *(fpx(ix,iy)-fpx(ix,iy-1))/dy/dy  &
                    - rhob(ix,iy)*ms*mw*kex(ix,iy)*rxn(ix,iy) 
                tmprl(1) = &
                    ! + poro(ix,iy)*rho(ix,iy)*mw*(1d0)/dt   &
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*(-1d0)/dx  &
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*(1d0)/dy  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(-1d0)/dx/dx  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(-1d0)/dy/dy  & 
                    - mw*(poro(ix,iy)*rho(ix,iy)*dif(ix,iy)-poro(ix,iy-1)*rho(ix,iy-1)*dif(ix,iy-1))*(1d0)/dy/dy  &
                    - rhob(ix,iy)*ms*mw*kex(ix,iy)*drxn_dfp(ix,iy) 
                tmprl(2) =  & 
                    + rhob(ix,iy)*ms*mw*kex(ix,iy)*drxn_dfp(ix,iy)
                tmprl(3) =    &
            ...
            if (ix==1) then 
                bx(row)= &
                    ...
                    + poro(ix,iy)*rho(ix,iy)*mw*(vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(fpx(ix,iy+1)-fpx(ix,iy))/dy  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix+1,iy)-1d0*fpx(ix,iy))/dx/dx  & 
                    - poro(ix,iy)*rho(ix,iy)*mw*dif(ix,iy)*(fpx(ix,iy-1)+fpx(ix,iy+1)-2d0*fpx(ix,iy))/dy/dy  & 
                    - mw*(poro(ix,iy)*rho(ix,iy)*dif(ix,iy)-poro(ix,iy-1)*rho(ix,iy-1)*dif(ix,iy-1))  &
                        *(fpx(ix,iy)-fpx(ix,iy-1))/dy/dy  &
                    - rhob(ix,iy)*ms*mw*kex(ix,iy)*rxn(ix,iy) 
                tmprl(1) = &
...
do iy=1,ny
    do ix = 1,nx
        ...
        if (frx(ix,iy)>=fri) rflxrxnpls = rflxrxnpls + rhob(ix,iy)*ms*mw*kex(ix,iy)*rxn(ix,iy)
        if (frx(ix,iy)< fri) rflxrxnmns = rflxrxnmns + rhob(ix,iy)*ms*mw*kex(ix,iy)*rxn(ix,iy)
...
print'(6E9.2)',rflxadv, rflxrxn, rflxt,  rflxadv+rflxrxn+rflxt, rflxrxnpls, rflxrxnmns
write(100,*) o18swi, rflxadv, rflxrxn, rflxt,  rflxadv+rflxrxn+rflxt, rflxrxnpls, rflxrxnmns
print*,'pflxadv, pflxrxn, pflxdif, pflxt, res '
print'(5E9.2)', pflxadv, pflxrxn, pflxdif, pflxt, pflxadv+pflxrxn+pflxdif+pflxt
write(200,*) o18swi, pflxadv, pflxrxn, pflxdif, pflxt, pflxadv+pflxrxn+pflxdif+pflxt
close(100)
close(200)
