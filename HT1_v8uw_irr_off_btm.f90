...
! from v8_irr: allowing off-axis flow?
implicit none
...
real(kind=8) tempk_pre(nx,ny),ds(nx,ny),presw(nx)
...
real(kind=8) :: tempb = 400d0 ! C bottom temp
...
real(kind=8) :: presw_0 = 25d6 !  Pa assuming 2.5 km depth 
...
real(kind=8) :: zscale = 250d0 ! scale depth for half decrease of permeability (see Fisher 1998) 
...
! real(kind=8),parameter :: temp_crit = 1200d0
real(kind=8),parameter :: y_crit = 6d3  
! real(kind=8),parameter :: y_crit = 50d3  
...
real(kind=8)::max_pres_dif=10d5  ! Pa (from 103 to 105 within 10 Ma from Fisher and Becker 2000)
! real(kind=8)::max_pres_dif=0d0  ! Pa (from 103 to 105 within 10 Ma from Fisher and Becker 2000)
real(kind=8)::presdif_half=xmax*0.5d0  ! 5Ma? from FB 20000
real(kind=8)::xscale=5d3 ! m dx  in FB 2000 
real(kind=8) obshflx
! 
...
workdir = trim(adjustl(workdir))//'perm_expexp_-16_8-11_8_zh300_wide_spx1_200x320_irr_btm400'
...
do iy = 1, ny
    do ix = 1,nx
    pres(ix,iy) = presw_0 + ym(iy)*1d-3*10d6  ! 1km 10MPa
...
do ix = 1,nx
    presw(ix) = presw_0 + max_pres_dif*exp(-(xmax-xm(ix))/presdif_half)*sin(xm(ix)*pi/xscale)
    presw(ix) = presw_0 
    if (xm(ix)>=30d3 .and. xm(ix)<50d3) presw(ix) = presw_0 + max_pres_dif
    if (xm(ix)>=70d3 .and. xm(ix)<100d3) presw(ix) = presw_0 + max_pres_dif
    if (xm(ix)>=125d3 .and. xm(ix)<150d3) presw(ix) = presw_0 + max_pres_dif
    if (xm(ix)>=175d3 .and. xm(ix)<200d3) presw(ix) = presw_0 + max_pres_dif
    if (xm(ix)>=225d3 .and. xm(ix)<250d3) presw(ix) = presw_0 + max_pres_dif
    if (xm(ix)>=275d3) presw(ix) = presw_0 + max_pres_dif
enddo 
        ...
        if (iy == 1) then 
            ...
            if (ix==1) then 
                bx(row) = (poro(ix,iy)*rho(ix,iy)*cp(ix,iy)+(1d0-poro(ix,iy))*rhom*cpm)*(-tempk(ix,iy))/dt &
                    ! + (vmx(ix,iy)+abs(vmx(ix,iy)))*0.5d0*cp(ix,iy)*(-(tempm + 273.15d0))/dx &
                    ! - kappa*(tempm + 273.15d0)/dx/dx  &
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)*((tempb + 273.15d0))/dy(iy) &
                    - kappa*(tempb + 273.15d0)/dy(iy)/dy(iy)  &
                    - rhom*w*(tempm+273.15d0)*cpm/dx(ix) 
                tmprl(1) = (poro(ix,iy)*rho(ix,iy)*cp(ix,iy)+(1d0-poro(ix,iy))*rhom*cpm)*(1d0)/dt &
                    ! + (vmx(ix,iy)+abs(vmx(ix,iy)))*0.5d0*(cp(ix,iy)-cpm)/dx*(1d0)  &
                    + (vmx(ix,iy)-abs(vmx(ix,iy)))*0.5d0*(cp(ix+1,iy)-cp(ix,iy))/dx(ix)*(1d0)  &
                    ...
                    + (vmy(ix,iy)+abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)/dy(iy)*(1d0) &
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)*(-1d0)/dy(iy) &
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(cpm-cp(ix,iy))/dy(iy)*(1d0) &
                    - kappa*(-1d0/(0.5d0*(dy(iy)+dy(iy-1)))-1d0/dy(iy))/dy(iy)     &
                    - rhom*w*(-1d0)*cpm/dx(ix) 
                tmprl(2) = (vmx(ix+1,iy)+abs(vmx(ix+1,iy)))*0.5d0*cp(ix+1,iy)/dx(ix+1)*(-1d0)   &
                    - kappa*(1d0)/(0.5d0*(dx(ix)+dx(Ix+1)))/dx(ix+1)  
                tmprl(3) =  (vmy(ix,iy-1)-abs(vmy(ix,iy-1)))*0.5d0*cp(ix,iy-1)/dy(iy-1)*(1d0) &
        ...
        if (iy == 1) then 
            ...
            if (ix==1) then 
                ...
                flxadv(ix,iy) = flxadv(ix,iy) & 
                    ...
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*cp(ix,iy)*((tempb + 273.15d0)-tempk(ix,iy))/dy(iy)  &
                    + (vmy(ix,iy)-abs(vmy(ix,iy)))*0.5d0*(cpm-cp(ix,iy))*tempk(ix,iy)/dy(iy) 
                ...
                flxcnd(ix,iy) = flxcnd(ix,iy) & 
                    ...
                    - kappa*((tempb + 273.15d0)-1d0*tempk(ix,iy))/dy(iy)/dy(iy)  
...
if (abs(sum(flxt))/abs(sum(flxb))<1d-6) exit 
...
write(100,*)'flxt, flxadv, flxcnd, flxb, flxres, obshflx'
write(100,*) sum(flxt), sum(flxadv), sum(flxcnd), sum(flxb), sum(flxres),obshflx
close(100)
